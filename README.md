...
1. **Host** — Tap **Gumawa ng inuman**, add at least 2 names, then **Simulan na!**
2. Server creates a **4-character room code** (e.g. `B7KQ`) shown on the tracker screen.
3. **Players** — Open the same URL, tap **Sumali sa code**, enter the code, pick their name from the list, then **Sumali na!**
4. When it's your turn, tap **Ininom ko na! 🥃** — everyone in the room sees the update instantly.
5. **Host** can add/remove drinkers, end the session (**Tapusin ang inuman**), or share the room code anytime.
6. On refresh/reconnect, your session is restored from browser storage when the room still exists.
